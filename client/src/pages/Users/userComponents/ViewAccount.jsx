...
function ViewAccount() {
  ...
  useEffect(() => {
    http
      .get("/trials/viewUserTrialReceipt")
      .then((res) => {
  ...
  useEffect(() => {
    http
      .get("/user/viewAccount/carListing")
      .then((res) => {
  ...
  useEffect(() => {
    http
      .get("/user/viewAccount/allFollowers")
      .then((res) => {
    ...
    <div className="dark:text-white text-neutral-600">
      <div className="mx-5 ">
        <Breadcrumb className="mb-5">
          ...
          <Breadcrumb.Item href="/user/viewAccount">My Account</Breadcrumb.Item>
        </Breadcrumb>
          ...
          <div className="my-3 flex flex-row h-fit">
            ...
            >
              ...
              <div className="mt-6 text-center">
                <div id="fullName" className="mb-3">
                  <h1 className="dark:text-green-400 text-sky-500 font-medium text-xl">
                    Full Name
                  </h1>
                  <p className="text-xl font-medium italic">{user.fullName}</p>
                </div>
                <div id="userName" className="mb-3">
                  <h1 className="dark:text-green-400 text-sky-500 font-medium text-xl">
                    Username
                  </h1>
                  <p className="text-xl font-medium italic">{user.userName}</p>
                </div>
                ...
                <div id="phoneNo" className="my-3">
                  ...
                  <p className="text-xl font-medium italic">{user.phoneNo}</p>
                </div>
                <Button
                  ...
                  onClick={() => navigate("/user/viewAccount/settings")}
                >
                  Settings
                </Button>
              </div>
            ...
            <div className="basis-4/6 ml-10 h-full">
              <Tabs.Group aria-label="Tabs with underline" style="underline">
                ...
                >
                  <div className="grid grid-cols-3 ">
                    {userCarSalesListing.length > 0 ? (
                      userCarSalesListing.map((userListing, i) => {
                        return (
                          <div
                            onClick={() =>
                              navigate(
                ...
                <Tabs.Item icon={BiReceipt} title="Receipt">
                  <div className="grid grid-cols-3">
                    {userReceiptList.length > 0 ? (
                      userReceiptList.map((userListing, i) => {
                        const formattedDate = format(
                          new Date(userListing.dateOfTrial),
                          "yyyy-MM-dd"
                        );
                        ...
                        return (
                          <div
                            key={userListing.trialReceiptId}
                            className="bg-slate-800 mr-2 mb-2"
                          >
                            <div className="p-5">
                              <p className="text-xs text-white/60 mb-4">
                                <span>{userListing.trialReceiptId}</span>
                              </p>
                              <p className="text-xl font-medium">
                                Model Name:{" "}
                              ...
                              {userListing.trialStatus === "Finished" ? (
                                <button
                                  onClick={() => {
                                    toggleModal(i),
                                      handleGiveFeedback(
                                  ...
                                  disabled={userListing.ratings !== null}
                                  title={
                                    userListing.ratings !== null
                              ...
                              >
                                <Modal.Header>
                                  Feedback for {userListing.modelName}
                                </Modal.Header>
                                <Modal.Body>
                                  <form onSubmit={formik.handleSubmit}>
                                    <div className="space-y-6">
                                      <p className="text-base leading-relaxed text-gray-500 dark:text-gray-400">
                                        On a scale, how did you enjoy the car?
                                        <div>
                                          <Rating
                                            value={parseFloat(
                ...
                <Tabs.Item icon={HiUserCircle} title="Followers">
                  <div className="grid grid-cols-3">
                        ...
                        return (
                          <div className="p-2 bg-slate-800 text-center rounded">
                            <a href={`/user/${followers.userName}`}>
                              <div className="px-8">
                                {followers &&
                                  followers.imageFile !== "No image" && ( // Had to be non-nullable
                                    <Avatar
                                      className="py-2"
                                      img={`${
                              ...
                              <p className="px-8 py-4">{followers.userName}</p>
                            </a>
                          </div>
                        );
                      })
